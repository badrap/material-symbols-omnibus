...
async function fetchSymbolNames() {
  ...
  const json: unknown = JSON.parse(text.slice(index));
  const data = v
    .object({
      icons: v.array(
        v.object({
...
async function main(concurrency = 15) {
  ...
  console.log(`// This is an autogenerated module. Please do not modify it manually.
export const data = ${JSON.stringify(sorted, null, 2)};
`);
}
